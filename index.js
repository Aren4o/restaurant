...
const app = {
	...
	callCenter(address, telNumber, isDelivery, isOpen, type){
		return `
			...
			${
				address = app.restaurantDetails[0].address !== app.restaurantDetails[0] ? 
				"Մեր հասցեն է" + app.restaurantDetails[0]: 
    ...
    staff(waiter,cook){
        return `
        ${
            cook = app.restaurantStaff[9].cook !== undefined ? 
            "Խոհարար" + app.restaurantStaff[9].cook + "այսօր աշխատանքի է, կարող եք պատվեր տալ" :
            "Խոհարար" + app.restaurantStaff[9].cook + "այսօր աշխատանքի չէ"
        }`
    }
};
